...
> Policy hash `45300d6f6cd721ff` — every verdict record carries the hash of the
...
Always fill `autoFixRationale` with one line on why this tier (e.g. "confirmed from source, bounded guard in editorOptions.ts, failing test named" · "confirmed from source; caller-vs-callee placement is an implementation choice, default noted in the trace" · "aligned feature with two viable designs — needs a maintainer's choice" · "feature request with no demand beyond the reporter").
